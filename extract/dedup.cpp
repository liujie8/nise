...
void getFileName(std::string path, std::vector<std::string>& files)
{
	...
	while(is >> name)files.push_back(name);
...
int save(std::ofstream &os)
{
	...
	for(iter = finger.begin();iter != finger.end();iter++)
	{
		...
		for(int i=0; i<size; i++)
			os << iter->second.urls[i].c_str() << std::endl;
		os << std::endl;
...
int saveImage()
{
	...
	for(iter = finger.begin();iter != finger.end();iter++)
	{
		...
		sprintf(dir,"ocr_result//%ld", iter->first);
	...
	//	system(cmd);
		...
		for(int i=0; i<size; i++)
		{
			...
			sprintf(cmd, "cp -r %s %s", iter->second.urls[i].c_str(), dir);
			printf("%s",cmd);
		//	system(cmd);
		}
		cnt++;
...
unsigned long long getFP(IplImage *img)
{
	...
	for(int i=0;i<4;i++)
		for(int j=0;j<4;j++)
		{
			...
			s.val[0] /= 16;
			pixel = (unsigned)s.val[0];
			if(pixel > 15)pixel=15;

			fingerprint = fingerprint * 16 + pixel;
		}
...
/*
	for(int i=0;i<5;i++)
		...
		{
			s = cvGet2D(fingerImage, i,j);
			var += (s.val[0] - mean)*(s.val[0] - mean);
		}

	var = sqrt(var/40);

	unsigned char v = (unsigned char)var;
	fingerprint = v;
...
//	fingerprint += m;
	...
	unsigned short su = (unsigned short)sum;
	sum /= 16;
	fingerprint += su;
...
//			unsigned long long fingerprint = getFP(img);
...
int main(int argc,char **argv)
{
	if(argc != 3)
	{
		cout << "Usage: dedup <input file> <result>" << endl;
		return 0;
	}
...
//	getFileName(init_path, img_urls);
...
//	n = saveImage();
//	std::cout << n << " images saved." << std::endl;
