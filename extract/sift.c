...
#define atd(dbinx,dbiny,dbint) *(dpt + (dbint)*binto + (dbiny)*binyo + (dbinx)*binxo)
  ...
  for(dyi =  VL_MAX (- W, 1 - yi    ) ; 
    ...
    for(dxi =  VL_MAX (- W, 1 - xi    ) ; 
      ...
      int dbinx_min = VL_MAX(0, -(NBP/2) - binx);
      int dbinx_max = VL_MIN(2, +(NBP/2) - binx);
      int dbiny_min = VL_MAX(0, -(NBP/2) - biny);
      int dbiny_max = VL_MIN(2, +(NBP/2) - biny);
      
      /* Distribute the current sample into the 8 adjacent bins*/
      for(dbinx = dbinx_min ; dbinx < dbinx_max ; ++dbinx) {
        for(dbiny = dbiny_min ; dbiny < dbiny_max ; ++dbiny) {
