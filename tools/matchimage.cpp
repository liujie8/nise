...
int main(int argc, char ** argv)
{
	if(argc != 10)
	{
		cout << "Usage: matchimage <src> <src_size> <dst> <dst_size> <hamming dist> <method> <scale> <quautize num> <max src sift>" << endl;
		return 0;
	}

	FeatureExtractor xtor1,xtor2;
	ImageLoader loader1,loader2;

	xtor1.setMaxImageSize(atoi(argv[2]));
	xtor2.setMaxImageSize(atoi(argv[4]));
	
	float scale = atof(argv[7]);
	float quaut = atof(argv[8]);
	int max_sift = atoi(argv[9]);
	...
	if(loader1.loadImage(s1.c_str(),true))
	{
		xtor1.setImage(loader1.getImg(0));
		switch(method)
		{
			case 0:
			xtor1.getBitFeat(f1, LDAHASH, scale, quaut, max_sift);
			break;
			case 1:
			xtor1.getBitFeat(f1, SKETCH, scale, quaut, max_sift);
			break;
			case 2:
			xtor1.getBitFeat(f1, LDATEST, scale, quaut, max_sift);
			break;
			case 3:
			xtor1.getBitFeat(f1, TEXTURETEST, scale, quaut, max_sift);
			break;
			default: break;
		}
	}
	...
	if(loader2.loadImage(s2.c_str(),true))
	{
		xtor2.setImage(loader2.getImg(0));

		switch(method)
		{
			case 0: 
				xtor2.getBitFeat(f2, LDAHASH, scale, quaut);
				break;

			case 1:       
				xtor2.getBitFeat(f2, SKETCH, scale, quaut);
				break;
			case 2:
				xtor2.getBitFeat(f2, LDATEST, scale, quaut);
				break;
			case 3:
				xtor2.getBitFeat(f2, TEXTURETEST, scale, quaut);
				break;
			default : break;
		}
	}
	...
	for(int i=0;i<f1.size();i++)
		...
		{
			if((d = hamming(f1[i].f.sketch,f2[j].f.sketch)) <= dist)
			{
				total += d;
	...
	ImageLoader loader3,loader4;
//	loader3.loadImage(argv[1]);
//	loader4.loadImage(argv[3]);

	IplImage * top = NULL;
	IplImage * bottom = NULL;
	
	if(loader3.loadImage(argv[1]) && loader4.loadImage(argv[3]))
	{
		top = loader3.getImg(0);
		bottom = loader4.getImg(0);
	}
